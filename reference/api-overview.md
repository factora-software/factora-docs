...
- **Live-Schlüssel** beginnen mit `fa_live_`, **Sandbox-Schlüssel** mit
  `fa_test_`.
- Schlüssel werden **nur als Hash** gespeichert und der Klartext **nur einmal**
  bei der Erstellung angezeigt.
- Optional pro Schlüssel: **Ablaufdatum**, **IP-Whitelist** (leer = überall
  gültig) und **Scopes** (leer = alle Rechte).
- Schlüssel lassen sich **rotieren** (neuen erzeugen, alten mit Übergangsfrist
  auslaufen lassen), deaktivieren oder löschen. Verwaltung erfolgt im Portal.
...
Die API ist pro Schlüssel auf ein Tageskontingent begrenzt. Wird es
überschritten, antwortet die API mit **HTTP 429** und einem `Retry-After`-
Header. Das Kontingent richtet sich nach dem gebuchten Tarif.
...
Das **Fehlerobjekt** enthält `code` und `message` immer, die übrigen Felder
je nach Fehlerart: `severity`, `field` (Eingabepfad), `rule`
...
| HTTP | Code | Bedeutung |
|---|---|---|
| 400 | `schema` | Ungültiges JSON, fehlende/falsche Felder, Summen-Abweichung |
...
| 401 | `authentication` | Schlüssel fehlt/ungültig/abgelaufen, IP nicht erlaubt |
| 402 | `limit_reached` | Rechnungslimit des Tarifs erreicht |
| 403 | `permission` | Kein API-Zugang / Sandbox-Schlüssel auf unzulässigem Endpunkt |
...
| 429 | `rate_limited` | Tageskontingent überschritten |
